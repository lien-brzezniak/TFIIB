...
all_genes <- read_tsv('/home/lien/data/skrypty/TFIIB/proten_coding_genes.bed') %>%
...
create_Groups <- function(group_sets,down_thresh=0,up_thresh=50000,complement_rest=F,exclusive=F){
  ...
  Groups <- lapply(names(group_sets),function(set){
    #set=names(group_sets)[[1]]
    #browser()
    grupy=group_sets[[set]]
    # browser()
    if (set %in% c('promoter','exon','intron','downstream','geneBody')) {
      Groups <- readRDS('/home/lien/data/data_mining/PCSD_database/protein_coding_genes/PCSD_allData.RDS')[[set]]
      #Groups <- Groups[which(names(Groups) %in% paste0('At_genes_S',c(3,11,26,31,32)))]
  ...
  if(exclusive){
    ...
    Groups2 <- Groups[which(!names(Groups)=='All')]
  ...
  if (complement_rest){
    Groups$rest <- all_genes$GeneID[which(!all_genes$GeneID %in% unlist(Groups[which(!names(Groups)=='All')]))]
  }
  ...
  new_names <- sub('Excl_At_genes_S','State ',new_names)
  names(Groups) <- new_names
  return(Groups)
}

create_meta_pos <- function(reach,res=100){
  tibble(label=c(as.character(seq(-4900,-100,100)),'TSS',as.character(seq(100,reach,100)),
         ...
         rel=if_else(is.na(relTSS),-1*relTTS,relTSS))
}

calculate_metamatrix <- function(genes,sample,reach,res=100){
  
  meta_pos=create_meta_pos(reach,res)
  matrix_raw_sum <- matrix(data = 0, nrow = nrow(meta_pos), ncol = nrow(meta_pos))
  matrix_norm_sum <- matrix(data = 0, nrow = nrow(meta_pos), ncol = nrow(meta_pos))
  matrix_count <- matrix(data = 0, nrow = nrow(meta_pos), ncol = nrow(meta_pos))
  #for (c in 1:5){
  # c=1
  dir='/home/lien/data/TFIIB/GEO_upload/HiC/'
  data <- readRDS(file=paste0(dir,'GMatrix_Sample',sample,'.RDS'))
  data <- data[which(names(data) %in% genes)]
  if(length(data)==0){next}
  genes_an <- names(data)
  TTSpos <- all_genes %>%  subset(GeneID %in% genes_an) %>%
    transmute(GeneID, 
              TTSrel=floor((end-start)/res)*res,
              blank_radius=if_else(TTSrel/2 >= reach,0, ceiling((reach-0.5*TTSrel)/res)))
  table_count <- lapply(meta_pos$pos, function(x){
    rel=meta_pos$rel[which(meta_pos$pos==x)]
    tibble(count=length(which(TTSpos$TTSrel>2*rel)),pos=x)
  }) %>% reduce(bind_rows)
  matrix_count <- matrix(data = 0, nrow = nrow(meta_pos), ncol = nrow(meta_pos))
  for(k in 1:nrow(matrix_count)){
    for(l in 1:ncol(matrix_count)){
      matrix_count[k,l] <- min(table_count$count[k],table_count$count[l])
    }}
  ...
  for (n in 1:nrow(TTSpos)){
    ...
    data_s <- data[[GeneID]]%>%
      ...
      transmute(relTSS1,relTSS2,norm_signal,raw_signal,
                relTTS1=relTSS1-geneLength, 
                relTTS2=relTSS2-geneLength) %>%
      subset(relTSS1<=reach | relTTS1>=-1*reach) %>%
      subset(relTSS2<=reach | relTTS2>=-1*reach) %>%
      mutate(relTSS1=if_else(relTSS1<=reach,relTSS1,NULL),
             relTSS2=if_else(relTSS2<=reach,relTSS2,NULL),
             relTTS1=if_else(relTTS1>=-1*reach,relTTS1,NULL),
             relTTS2=if_else(relTTS2>=-1*reach,relTTS2,NULL)) %>%
      left_join(meta_pos[,2:4], by=c('relTSS1'='relTSS','relTTS1'='relTTS')) %>%
      transmute(meta1=pos, raw_signal, norm_signal, relTSS2,relTTS2) %>%
      left_join(meta_pos[,2:4], by=c('relTSS2'='relTSS','relTTS2'='relTTS')) %>%
      transmute(meta2=pos, meta1,raw_signal, norm_signal) %>%
      subset(!is.na(meta1) & !is.na(meta2))
  ...
  matrix_raw_mean=matrix_raw_sum/matrix_count
  matrix_norm_mean=matrix_norm_sum/matrix_count
  matrix_norm_mean[which(meta_pos$label=='GeneBody'),] <- NA
  matrix_norm_mean[,which(meta_pos$label=='GeneBody')] <- NA
  matrix_raw_mean[which(meta_pos$label=='GeneBody'),] <- NA
  matrix_raw_mean[,which(meta_pos$label=='GeneBody')] <- NA
  
  rownames(matrix_raw_mean)=meta_pos$label
  colnames(matrix_raw_mean)=meta_pos$label
  rownames(matrix_norm_mean)=meta_pos$label
  colnames(matrix_norm_mean)=meta_pos$label
  
  list(mat_raw=matrix_raw_mean, mat_norm = matrix_norm_mean, nr_of_genes = max(matrix_count))
...
f_draw_matrix <- function(raw_array=NULL, norm_array=NULL, delta_array=NULL,add_frame=T, do_scaling=T,title='Metagene Matrix', threshold=0.98, max_scale_raw=NULL, max_scale_norm=NULL, max_scale_delta=NULL){
 # browser()
  ...
  if(!is.null(delta_array)){
    ...
    if(add_frame){
       if(is.na(sum(delta_array,na.rm=F))) {outRegion=101} else {outRegion=100 }
     meta_pos<-create_meta_pos(reach=(nrow(delta_array)-outRegion)/2*100)
...
bootANDplot <- function(data,Groups,merge_samples){
  #Groups -  a named list of character vectors containing AGI gene identifiers
  #merge_samples - a int vector containing numbers of samples from which data for each gene should be averaged before bootstrap
  library(tidyverse)
  library(ggplot2)
  library(boot)
  library(scico)
...
report_contact_summary <- function(contact_summary,Groups){
  sectors_plots <-lapply(names(Groups),function(G){
    ...
    ss %>% #ggplot(aes(x=stress,y=bla))+geom_point()
      ggplot(aes(x=stress,y=value,color=genotype,fill=genotype))+
      #scale_color_hue(direction = -1, h.start=60,aesthetics = c("color","fill"))+
      geom_col(width=0.9,position=position_dodge2(padding=0.3),size=0.8 )+
      #geom_jitter(aes(group=genotype,shape=plants),
      # color='black',
      # position=position_jitterdodge(dodge.width=1,jitter.height=0))+
...
kolory=c("Wt"="#3b3b3b","R58C"="#c87970")
kolory2=c("<25%"="#191d57","25-75%"="#949496",">75%"="#c1841e")
#show_col(kolory)
#outdir=paste0('/home/lien/data/TFIIB/figury/Fig4/')
