###  This code reproduces the analysis of 3'RNAseq data (Fig. 1 and 2)
```{r}
...
####   Fig. 1C - multifactorial analysis - PCA plot
```{r}
...
####  Identification of genes differentially expressed in TFIIB1-R58C compared to TFIIB1-WT
```{r}
...
All_DEGs_list <-purrr::reduce(DEGs_tables,bind_rows) %>%
  select(GeneID) %>%
  distinct() 
...
All_DEGs_list
```
####  Fig.2 Trainability Index
#####  calculate mean rpm values for all conditions and genotypes (average from 3 biological repeats); filter out lowly expressed genes - reject genes that had mean rpm less than 5 in any experimental condition 
```{r}
...
arranged$Order <- 1:nrow(arranged)
arranged
```
...
#### Fig. 2B upper panel
```{r}
ggplot(arranged, aes(x=Order, y=Wt,color=Type_L)) + 
...
#####  transform counts table to rpm table  
```{r}
rpm <- nor %>%
  mutate_if(is.numeric, rpm_fun)
```

#####  Plot the 30 most trainable genes:
```{r}
single_plots <- lapply(c(1:30),function(x){
  gene=arranged$GeneID[x]
  TrI=arranged$Order[x]
  plot_gene <- rpm %>%
