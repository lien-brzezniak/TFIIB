...
max_norm = quantile(abs((unlist(matrices))),probs=threshold,na.rm=T)
  ...
  pdf_file=paste0(output_path,paste0(names(Groups),collapse='_'),'SuppFig4A.pdf')
  pdf(pdf_file)
lapply(names(Groups),function(group){ 
  ...
  lapply(samples,function(sample){
    ...
    f_draw_matrix(norm_array=norm_map, threshold=1,title=title,do_scaling=F,max_scale_norm=max_norm)
  })
})
