...
sample_info
# see comments in functions.R::create_Groups() for other defined gene-groups. A custom list of genes may also be provided.
...
#provide the id of analysed sample (according to samples_GEO_HiC.tsv), 1 to 12
s='1'
...
lapply(metamatrix_data, function(x){
  ...
  f_draw_matrix(raw_array=x$mat_raw,norm_array=x$mat_norm,do_scaling=T,title=title,threshold=0.995)
})
...
lapply(metamatrix_data, function(x){
  title=paste0(x$sample,'   ',x$name,'\n',x$group,'; ',down_thresh/1000,' kb to ',up_thresh/1000,' kb\n',x$nr_of_genes,' genes')
  f_draw_matrix(raw_array=x$mat_raw,norm_array=x$mat_norm,do_scaling=T,title=title,threshold=0.995)
  write.table(x$mat_raw, file=paste0(output_path,'Meta_raw_Sample',s,'_',x$group,'.txt'), sep='\t', row.names = F, col.names =  F)
  write.table(x$mat_norm, file=paste0(output_path,'Meta_norm_Sample',s,'_',x$group,'.txt'), sep='\t', row.names = F, col.names =  F)
})
dev.off()
